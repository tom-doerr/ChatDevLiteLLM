...
class OpenAIEmbedding:
    ...
    def get_text_embedding(self,text: str):
            if len(text)>8191:
                  text = text[:8190]
            response = litellm.embedding(input = text, model="text-embedding-ada-002").model_dump()
    ...
    def get_code_embedding(self,code: str):
            if len(code) == 0:
                  ...
                  code = code[0:8190]
            response = litellm.embedding(input=code, model="text-embedding-ada-002").model_dump()
            embedding = response['data'][0]['embedding']
            log_and_print_online(
